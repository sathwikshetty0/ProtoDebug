    ...
    <div className="rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800 shadow-sm animate-scale-up group relative">
      <div className="code-block-header !bg-gray-800 dark:!bg-black px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
           <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{lang || 'CODE'}</span>
        </div>
        <button 
          onClick={handleCopy}
      ...
      <div className="relative">
        <pre className="code-block-body !bg-gray-900 !p-6 text-[12px] leading-relaxed selection:bg-indigo-500/30">
          {code}
        </pre>
        <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
           <span className="text-[10px] font-black text-white/10 uppercase tracking-widest">{code.split('\n').length} LINES</span>
        </div>
      </div>
    </div>
  );
